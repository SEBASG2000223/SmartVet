function limpiarForms() {
    $('#customer_add').trigger('reset');
    $('#customer_update').trigger('reset');
}
...
function listarClientesTodos() {
    tabla = $('#tbllistado').dataTable({
        ...
        ajax: {
            url: '../controllers/customerController.php?op=listar_para_tabla',
            type: 'get',
            dataType: 'json',
            error: function (e) {
...
$('#customer_add').on('submit', function (event) {
    event.preventDefault();
    $('#btnRegistrar').prop('disabled', true);
    var formData = new FormData($('#customer_add')[0]);
    $.ajax({
        url: '../controllers/customerController.php?op=insertar',
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
...
$('#tbllistado tbody').on('click', 'button[id="modificarCliente"]', function () {
    ...
    $('#EId').val(data[0]);
    $('#Eemail_address').val(data[1]);
    $('#Efull_name').val(data[2]);
    return false;
});

$('#customer_update').on('submit', function (event) {
    event.preventDefault();
    bootbox.confirm('¿Desea modificar los datos?', function (result) {
        if (result) {
            var formData = new FormData($('#customer_update')[0]);
            $.ajax({
                url: '../controllers/customerController.php?op=editar',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
